...
def get_select_inds(N_samples, iterations, random_scale=True, random_shift=True):
    ...
    w, h = torch.meshgrid(
        [torch.linspace(-1, 1, N_samples_sqrt), torch.linspace(-1, 1, N_samples_sqrt)]
    )
    h = h.unsqueeze(2)
    w = w.unsqueeze(2)
...
def create_nerf(args):
    ...
    # Create optimizer
    train_params = [
        {"params": model_fine.rgb_linear.parameters()},
        {"params": model.rgb_linear.parameters()},
    ]
    if args.use_alpha:
...
def config_parser():
    ...
    parser.add_argument(
        "--i_weights", type=int, default=2000, help="frequency of weight ckpt saving"
    )
    parser.add_argument(
        "--i_testset", type=int, default=200000, help="frequency of testset saving"
    )
    parser.add_argument(
        "--i_video",
        type=int,
    ...
    parser.add_argument(
        "--description",
        type=str,
        default="A green excavator",
        help="the text that guides the editing/generation (Or: A photo of a XXX excavator)",
    )
    parser.add_argument(
        "--src_description",
        type=str,
        default="An outline of an excavator with black background",
        help="the text that describes the image",
    )
    parser.add_argument("--use_clip", action="store_true", help="whether use clip loss")
    parser.add_argument(
        "--use_alpha", action="store_true", help="whether finetune alpha layers"
    )
    parser.add_argument(
        "--use_feature", action="store_true", help="whether finetune feature layers"
    )
    parser.add_argument(
        "--use_view", action="store_true", help="whether finetune view layers"
    )
    parser.add_argument(
        "--w_clip", type=float, default=1.0, help="weight for CLIP loss"
    )
    parser.add_argument(
        "--sample_scale",
        type=int,
        default=60,
        help="sample scale for patch-based samplar",
    )
    return parser
...
def train():
    ...
    src_text = torch.cat([clip.tokenize(args.src_description)]).to(device)
    clip_loss = CLIPLoss()
    ...
    for i in trange(start, N_iters):
        ...
        if use_batching:
            ...
            rays_d = torch.nn.functional.grid_sample(
            ...
            mask_s = mask_s.permute(1, 2, 0).view(-1, 1)
        ...
        mask_img = mask_s.view(sample_scale, sample_scale, -1)
        ...
        mask_img = mask_img.permute(2, 0, 1).unsqueeze(0)
        ...
        if args.use_clip:
            ...
            if "rgb0" in extras:
                gen_img_rgb0 = (
                    extras["rgb0"]
                    .view(sample_scale, sample_scale, -1)
                    .permute(2, 0, 1)
                    .unsqueeze(0)
                )
        ...
        if i % args.i_print == 0:
            if args.use_clip:
                tqdm.write(
                    f"[TRAIN] Iter: {i} Loss: {loss.item()}  CLoss: {c_loss.item()} CLossRGB0: {c_loss_rgb0.item()} PSNR: {psnr.item()} LR: {new_lrate} GLOBAL_STEP: {global_step} i: {i} Start: {start}"
                )
            else:
